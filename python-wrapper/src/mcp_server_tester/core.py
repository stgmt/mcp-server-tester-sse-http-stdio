...
class MCPTester:
    ...
    def _prepare_config(self, config: Union[str, Path, Dict[str, Any]], config_type: str) -> str:
        """Prepare configuration file path or create temporary file from dict."""
        if isinstance(config, (str, Path)):
            config_path = Path(config)
            if not config_path.exists():
                raise MCPConfigurationError(f"{config_type} file not found: {config_path}")
            ...
            raise MCPConfigurationError(f"Invalid {config_type} type. Expected str, Path, or dict.")
    
    def _parse_result(self, result: subprocess.CompletedProcess, format: str) -> MCPTestResult:
        ...
        if format == "json":
            try:
                ...
                return MCPTestResult(
                    ...
                    total_tests=1,
                    failed_tests=[] if result.returncode == 0 else [{"error": "JSON parsing failed"}],
                    execution_time=0.0,
                    output=result.stdout,
                    error=result.stderr
                )
            ...
            return MCPTestResult(
                ...
                passed_tests=1 if result.returncode == 0 else 0,
                total_tests=1,
                failed_tests=[] if result.returncode == 0 else [{"error": "Test failed"}],
                execution_time=0.0,
                output=result.stdout,
                error=result.stderr
            )
    
    def list_tools(self, server_config: Union[str, Path, Dict[str, Any]], server_name: Optional[str] = None) -> List[Dict[str, Any]]:
        """List available tools from MCP server."""
        # This would use the tools listing functionality
        cmd = ["npx", self.npm_package_name, "tools"]
        cmd.extend(["--server-config", self._prepare_config(server_config, "server_config")])
        if server_name:
            cmd.extend(["--server-name", server_name])
        
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            raise MCPTestExecutionError("Failed to list tools", result.returncode, result.stderr)
