...
@click.option(
    "--timeout", default=30000, help="Timeout in milliseconds (default: 30000)"
)
...
@click.option(
    ...
    "-f",
    default="json",
    type=click.Choice(["json", "junit", "tap"]),
    help="Output format",
)
...
def create_server_config(output: Optional[str]):
    ...
    while True:
        ...
        transport = click.prompt(
            "Select transport", type=click.Choice(["sse", "stdio", "http"])
        )
        ...
        if transport == "sse":
            ...
            args = click.prompt(
                "Enter arguments (space-separated, optional)", default=""
            )
    ...
    while True:
        ...
        while True:
            tool_name = click.prompt(
                "Enter tool name to test (or 'done' for this test)"
            )
            ...
            # Add arguments
            has_args = click.confirm(
                "Does this tool call have arguments?", default=False
            )
            if has_args:
